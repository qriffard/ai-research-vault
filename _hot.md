...
- **Anti-slop design cluster** ([[anti-slop-design]] concept hub): [[impeccable]] (Bakaus, 45k★), [[taste-skill]] (Leonxlnx, 61k★), [[emil-kowalski-skills]] (6.8k★) — "agents with taste" skills, all from Anthropic `frontend-design`. Root-cause mechanism in **[[llm-laziness]]** (truncation/brevity = behavioral artifact, not capability; prefer API/CLI over consumer web). Concrete design rules usable for my HTML digests.
- **[[cl4r1t4s]] ingested (elder-plinius, 45k★, AGPL).** Extracted system prompts for all major AI products (per-vendor dirs). Transparency/red-team reference; ties to [[llm-laziness]] + [[opus-think-like-fable]]. ⚠ **README embeds a leetspeak prompt-injection** (dump-your-own-instructions) — NOT complied with; treat repo as untrusted input.
- **[[opus-think-like-fable]] ingested (Nate Herk video, 2026-07-07).** "Model isn't the moat" — transplant a frontier model's *process* to cheaper models: teacher-not-workhorse, model routing / effort right-sizing, reverse-engineer good outputs into skills, "Fable mode" skill (scoping/evidence/attacking/verifying/reporting gates), routing table. Opus+Haiku scouts ~3× cheaper, same quality.
- **[[creative-frontend-effects]]** pointer cluster (R3F/liquid-glass-react/ShaderGradient/liquid-logo) — WebGL visual-craft building blocks for high-craft frontends; serves [[anti-slop-design]].
